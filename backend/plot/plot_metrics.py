...
def plot_all(csv_path, output_dir):
    ...
    ax_all.set_title("Batch-by-Batch Learning Rate Trend (First 5 Batch)", weight="bold", pad=12)
    ax_all.set_ylabel("Learning Rate", labelpad=10)
    ax_all.set_xlabel("Batch", labelpad=5)
    ...
    ax_zoom.axvline(2.0, color="#059669", linestyle="--", alpha=0.8, label="Warm Restart")
    ...
    ax_zoom.set_xlabel("Batch", labelpad=10)
    ...
    explanation_text = (
        "NOTE:\n"
        "1. The scheduler is set with T_0 = 2 batch and T_mult = 2.\n"
        "2. The learning rate oscillates smoothly and continuously within each epoch (batch-by-batch).\n"
        "3. The cycle lasts exactly 2 batch, reaching the minimum (eta_min = 1e-8) at the end of second batch.\n"
        "4. At the beginning of next batch, the restart to eta_max (5e-5) occurs.\n"
        "5. The second cycle lasts 2 * 2 = 4 batch (ending at the end of 6 batch)."
    )
...
if __name__ == "__main__":
    ...
    csv_path = os.path.abspath(os.path.join(script_dir,"../",".weights","v7_frozen","training_history.csv"))
