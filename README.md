...
Reviewer Agent è una piattaforma avanzata per l'analisi automatica e intelligente delle recensioni. Combina il meglio di tre mondi: **Deep Learning (Transformer)** per lo scoring qualitativo, **RAG (Retrieval-Augmented Generation)** per la conformità normativa e **LLM strutturati** per l'analisi grammaticale e stilistica.
...
Un modello basato su **DeBERTa-v3** (architettura Transformer all'avanguardia) addestrato con una strategia multi-task per prevedere un punteggio di "informatività".
...
*   **Architettura Multi-Head**:
    *   **Score Head**: Regressione 0-100 per il punteggio di insightfulness (distillato da un LLM "Teacher").
...
*   **Guideline Retriever**: Utilizza una strategia a due livelli (Keyword Match + Semantic Embedding) per trovare il documento di policy più pertinente in un database vettoriale locale.
*   **Compliance Checker**:
    ...
    *   **Check Contestuale**: Utilizza un LLM locale (**Gemma 3**) per interpretare se il testo viola regole specifiche e sfumate della piattaforma.

### 3. Analisi Grammaticale e Highlights
Un motore di analisi linguistica che identifica errori oggettivi e suggerisce miglioramenti stilistici.

*   **Highlighting Chirurgico**: Il sistema restituisce indici di carattere precisi (`start`, `end`) per ogni problema, permettendo al frontend di colorare il testo in modo interattivo.
...
## 📊 Dashboard dei Modelli e Benchmark

Il progetto include un sistema di benchmark (`backend/comparison/`) che confronta il modello locale con i giganti del settore:
*   **Performance**: Il modello locale `Insightfulness` è ~10x più veloce di Gemini 1.5 Flash (0.2s vs 2.1s per recensione).
*   **Correlazione**: Altissima correlazione (Pearson > 0.85) con i giudizi di modelli LLM top-tier (Ollama Cloud 120B e Gemini).
...
*   **LLM Engine**: Ollama (Gemma 3:27b)
*   **Orchestrazione**: LangChain
*   **NLP**: spaCy (solo fase di training)
...
*Sviluppato con cura per rendere l'ecosistema delle recensioni più trasparente e affidabile.*
